...
function fillTableBody(pageInfo) {
    $("#rolePageBody").empty(); //表格体清空
    $("#Pagination").empty(); //分页条清空

    //判断pageInfo是否有效
    if (pageInfo == null || pageInfo == undefined || pageInfo.list == null || pageInfo.list.length == 0) {
        $("#rolePageBody").append("<tr><td colspan='4'> 抱歉！没有查询到您搜索的数据！</td></tr>"); //在rolePageBody中展示
        return ;
    }
    ...
    for(var i=0; i<pageInfo.list.length ; i++){
        ...
        var numberTd = "<td>"+(i+1)+"</td>"; //角色名序号
        var checkboxTd = "<td><input roleId='"+roleId+"' type='checkbox' class='itemBox'></td>"; //复选框，携带roleId属性值，自定义
        var roleNameTD = "<td>"+roleName+"</td>";  //角色名
        ...
        var buttonTd = "<td>"+checkBtn+" "+pencilBtn+" "+removeBtn+"</td>"; //按钮组 ，也需要携带roleId属性值，自定义
        var tr = "<tr>"+numberTd+checkboxTd+roleNameTD+buttonTd+"</tr>"; //组合为一行的表格
        $("#rolePageBody").append(tr); //添加到库中
    }
